...
public class ShoppingCart {
     ...
     * incremented.
     *
     * @param dvd the <code>Product</code> that defines the type of shopping
     * cart item
     * @see ShoppingCartItem
     */
    public synchronized void addItem(Dvd dvd) {

        boolean newItem = true;

        for (ShoppingCartItem scItem : items) {

            if (scItem.getDvd().getId() == dvd.getId()) {
        ...
        if (newItem) {
            ShoppingCartItem scItem = new ShoppingCartItem(dvd);
            items.add(scItem);
        }
    }
     ...
     * @see ShoppingCartItem
     */
    public synchronized void update(Dvd dvd, String quantity) {
    ...
    public synchronized double getSubtotal() {
        ...
        for (ShoppingCartItem scItem : items) {
            ...
            Dvd dvd = (Dvd) scItem.getDvd();
            amount += (scItem.getQuantity() * dvd.getPrice().doubleValue());
        }
    ...
    /**
     * 
     * @return Map of items (dvds) which are not sufficient in the stock correspondent with their missed quantity
     */
    public HashMap<ShoppingCartItem,Integer> getMissedItemsInStock() {

        HashMap<ShoppingCartItem,Integer> missedMap = new HashMap<>();

        for (ShoppingCartItem item : this.items) {
            if (!item.isDisponible()) {
                missedMap.put(item,item.getMissedQuantity());
            }
        }
    ...
    public boolean isMissingInStock(){
    
        HashMap<ShoppingCartItem,Integer> missedMap = this.getMissedItemsInStock();
